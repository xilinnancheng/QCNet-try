...
class QCNetAgentEncoder(nn.Module):
    ...
    def forward(self,
        ...
        # [num_agent, historical_time_steps]
        mask = data['agent']['valid_mask'][:, :self.num_historical_steps].contiguous()
        # [num_agent, historical_time_steps, input_dim]
        ...
        # [num_agent, historical_time_steps, 2]
        head_vector_a = torch.stack([head_a.cos(), head_a.sin()], dim=-1)
        # [num_polygon, input_dim]
        ...
        if self.dataset == 'argoverse_v2':
            ...
            # [num_agent * historical_time_steps, embed_dim]
            categorical_embs = [
        ...
        if self.dataset == 'argoverse_v2':
            # [num_agent, historical_time_steps, 4]
            x_a = torch.stack(
        ...
        # [num_historical_step * num_agent, self.input_dim]
        pos_s = pos_a.transpose(0, 1).reshape(-1, self.input_dim)
        # [num_historical_step * num_agent, self.input_dim]
        head_s = head_a.transpose(0, 1).reshape(-1)
        # [num_historical_step * num_agent, 2]
        head_vector_s = head_vector_a.transpose(0, 1).reshape(-1, 2)
        # [num_historical_step * num_agent]
        mask_s = mask.transpose(0, 1).reshape(-1)
        # [num_historical_step * num_polygon, 2]
        pos_pl = pos_pl.repeat(self.num_historical_steps, 1)
        # [num_historical_step * num_polygon]
        ...
        for i in range(self.num_layers):
            # [num_agent * num_historical_steps, hidden_dim]
            x_a = x_a.reshape(-1, self.hidden_dim)
            x_a = self.t_attn_layers[i](x_a, r_t, edge_index_t)
            # [num_historical_steps * num_agent, hidden_dim]
